export const BISHOPRIC_SESSION_COOKIE = 'bishopric_session';

const SESSION_TTL_MS = 12 * 60 * 60 * 1000;
export const BISHOPRIC_SESSION_MAX_AGE_SECONDS = SESSION_TTL_MS / 1000;

const getSecret = (): string => {
    const secret = process.env.BISHOPRIC_PASSWORD;
    if (!secret) throw new Error('BISHOPRIC_PASSWORD is not set');
    return secret;
};
