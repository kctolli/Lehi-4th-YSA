export const WARD_COUNCIL_SESSION_COOKIE = 'ward_council_session';

const SESSION_TTL_MS = 12 * 60 * 60 * 1000;
export const WARD_COUNCIL_SESSION_MAX_AGE_SECONDS = SESSION_TTL_MS / 1000;

const getSecret = (): string => {
    const secret = process.env.WARD_COUNCIL_PASSWORD;
    if (!secret) throw new Error('WARD_COUNCIL_PASSWORD is not set');
    return secret;
};
